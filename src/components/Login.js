...
import { loginUser, loginUserWithGoogleAuth } from "../store/UserAction";

function Login() {
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const { buttonLoader, userRegistered, errors, userLoggedIn, authError } =
  ...
  useEffect(() => {
    if (authError) {
      ...
      dispatch(userActions.updateAuthErrorValue());
    }
  }, [authError, dispatch]);
  ...
  }, [userLoggedIn, navigate, dispatch]);

  useEffect(() => {
    if (userRegistered) {
      toast.success("you are registered");
    }
    dispatch(userActions.updateUserRegisteredValue());
    ...
    <>
      ...
      <div className="login-wrapper">
        <div className="login-container">
          <h1 id="title">Login for TUMMOC</h1>
